...
class Straight_layout extends CI_Driver
{
    ...
    {
        if (!isset(get_instance()->load->_views[0])) {
            return $output;
        }
        ...
        if (file_exists(VIEWPATH.get_instance()->load->_skin.EXT)) {
            $output = get_instance()->load->view(get_instance()->load->_skin, ['skin'=>$output], true, true);
        }
    ...
    private function layout($output = '')
    {
        ...
        if (file_exists(VIEWPATH.get_instance()->load->_layout.EXT)) {
            $output = get_instance()->load->view(get_instance()->load->_layout, ['layout'=>$output], true, true);
        } else {
    ...
    private function view2combineAsset($output)
    {
        ...
        if (count($_js)) {
            ...
            get_instance()->load->js("/{$asset_path}/combine/{$hash}.js?l=".urlencode(implode(',', $_js)));
        }
        ...
        if (count($_css)) {
            ...
            get_instance()->load->css("/{$asset_path}/combine/{$hash}.css?l=".urlencode(implode(',', $_css)));
        }
            ...
            if (is_array($href)) {
                $href = isset($href[0]) ?
                        ...
                        implode(' ', array_map(function ($key, $val) {
                            return "{$key}='{$val}'";
                        }, array_keys($href), array_values($href)));
            } else {
        ...
        foreach ($_js as $key => $src) {
            if (is_array($src)) {
                $src = isset($src[0]) ?
                    ...
                    implode(' ', array_map(function ($key, $val) {
                        return "{$key}='{$val}'";
                    }, array_keys($src), array_values($src)));
            } else {
    ...
    private function sanitize_output($buffer)
    {
        ...
        if ($cnt && $cnt > count(get_instance()->load->_js)) {
            $buffer = str_replace($found[0], array_map(function ($el) {
                return '<textarea>'.$el.'</textarea>';
            }, array_keys($found[0])), $buffer); // replacing both with <textarea>$index</textarea>
            $buffer = preg_replace(['/\>[^\S ]+/s', '/[^\S ]+\</s', '/(\s)+/s', '/<!--[^\\[<>].*?(?<!!)-->/'], ['>', '<', '\\1', ''], $buffer);
            $buffer = str_replace(array_map(function ($el) {
                return '<textarea>'.$el.'</textarea>';
            }, array_keys($found[0])), $found[0], $buffer); // Replacing back with content
        } else {
            $buffer = preg_replace(['/\>[^\S ]+/s', '/[^\S ]+\</s', '/(\s)+/s', '/<!--[^\\[<>].*?(?<!!)-->/'], ['>', '<', '\\1', ''], $buffer);
        }
